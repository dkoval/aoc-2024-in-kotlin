...
fun main() {
    ...
    fun part1(input: List<String>, minSavingTime: Int): Int {
        ...
        fun traverse(start: Cell): Int {
            fun bfs(start: Cell): Int {
                ...
                val visited = mutableSetOf<Cell>(start)
                var time = 0
                while (q.isNotEmpty()) {
                    repeat(q.size) {
                        val curr = q.poll()
                        for (d in Direction.entries) {
                            ...
                            if (grid[next.row][next.col] == 'E') {
                                return time + 1
                            }

                            if (grid[next.row][next.col] != '#' && next !in visited) {
                                q.offer(next)
                                visited += next
                            }
                        }
                ...
                error("Cold not reach the ending cell")
            }

            val baseTime = bfs(start)
            ...
            for (row in 1 until m - 1) {
                for (col in 1 until n - 1) {
                    if (grid[row][col] == '#') {
                        grid[row][col] = '.'
                        if (baseTime - bfs(start) >= minSavingTime) {
                            count++
                        }
                        grid[row][col] = '#'
                    }
        ...
        return error("Cold not find the starting cell")
    }

    fun part2(input: List<String>, maxCheatingTime: Int, minSavingTime: Int): Int {
        val grid = input.map { line -> line.toMutableList() }
        val m = grid.size
        val n = grid[0].size
        ...
        fun traverse(start: Cell, end: Cell): Int {
            ...
            for ((c1, t1) in times1) {
                for ((c2, t2) in times2) {
                    ...
                    // cheatingTime - the time it takes to get from (row1, col1) to (row2, col2) regardless of obstacles
    ...
    check(part1(testInput, 2) == 42)
    check(part2(testInput, 20, 50) == 285)

    val input = readInput("day$DAY_ID/Day$DAY_ID")
    part1(input, 100).println()     // answer 1375
    part2(input, 20, 100).println() // answer 983054
}
